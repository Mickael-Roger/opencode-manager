...
- Made detaching from a workspace non-destructive. The container runs OpenCode in a TTY, so Ctrl-C previously reached OpenCode and exiting it stopped the container's main process, killing the workspace. Attach now runs with `--detach-keys=ctrl-c`, so the attach client intercepts Ctrl-C as its detach sequence: OpenCode never sees it, the container keeps running in the background, and re-attaching reconnects to the same session.
