...
case "$ref" in
  ...
  tools/grafana)    ocm_env=(OCM_URL=https://grafana.example.com OCM_TOKEN=ci-token); verify='command -v gcx && command -v dashboard-linter' ;;
  tools/harbor)     ocm_env=(OCM_IMPORT_CONFIG=false); verify='command -v harbor' ;;
